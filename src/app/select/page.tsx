    ...
    <div className="flex flex-col justify-between h-full lg:pr-18">
      ...
      <div>
        ...
        <form className="grid gap-4 sm:grid-cols-3">
          {/* Arcade */}
          <div
            onClick={() => handleSelect("Arcade", billing === "yearly" ? "$90/yr" : "$9/mo")}
            className={`cursor-pointer border rounded-lg p-4 hover:border-blue-400 ${
              formData.plan === "Arcade" ? "border-blue-500 bg-blue-50" : "border-gray-300"
            }`}
          >
            <Image src="/images/icon-arcade.svg" alt="Arcade" width={40} height={40} />
            <h3 className="font-bold mt-7">Arcade</h3>
            <p>{billing === "yearly" ? "$90/yr" : "$9/mo"}</p>
            {billing === "yearly" && <span className="text-xs text-blue-600">2 months free</span>}
          </div>

          {/* Advanced */}
          <div
            onClick={() => handleSelect("Advanced", billing === "yearly" ? "$120/yr" : "$12/mo")}
            className={`cursor-pointer border rounded-lg p-4 hover:border-blue-400 ${
              formData.plan === "Advanced" ? "border-blue-500 bg-blue-50" : "border-gray-300"
            }`}
          >
            <Image src="/images/icon-advanced.svg" alt="Advanced" width={40} height={40} />
          ...
          {/* Pro */}
          <div
            onClick={() => handleSelect("Pro", billing === "yearly" ? "$150/yr" : "$15/mo")}
            className={`cursor-pointer border rounded-lg p-4 hover:border-blue-400 ${
              formData.plan === "Pro" ? "border-blue-500 bg-blue-50" : "border-gray-300"
            }`}
          >
            <Image src="/images/icon-pro.svg" alt="Pro" width={40} height={40} />
        ...
        <div className="flex items-center justify-center gap-4 bg-gray-100 mt-6 p-2 rounded-lg">
          <span className={billing === "monthly" ? "font-bold text-blue-600" : ""}>Monthly</span>
          ...
          <span className={billing === "yearly" ? "font-bold text-blue-600" : ""}>Yearly</span>
        </div>
      </div>
